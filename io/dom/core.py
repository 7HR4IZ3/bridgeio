...
class DOMTokenList(list):
    ...
    def add(self, *args):
        ...
        for item in args:
            if item not in self:
                self.append(item)
                self.__el.className = self.toString()
...
class NodeList(list):
    ...
    def entries(self) -> t.Iterable[t.Tuple[int, Node]]:
    ...
    def values(self) -> t.Iterable[Node]:
...
class Document(Element):
    ...
    @staticmethod
    def createEvent(event_type=None, *args, **kwargs):
        """[Creates a new event]
        ...
        if event_type == "MouseEvent":
            return MouseEvent(*args, **kwargs)
        elif event_type == "KeyboardEvent":
            return KeyboardEvent(*args, **kwargs)
        elif event_type is None:
            return Event(*args, **kwargs)
        return Event(event_type, *args, **kwargs)
    ...
    def write(self, html: str = ""):  # -> None: #Â TODO - untested
    ...
    def writeln(self, html: str = ""):  # -> None: #Â TODO - untested
...
class HTMLCollection(list):
    ...
    def __getitem__(self, index):
        ...
        # index = "named.item.with.periods" #Â TODO - test
        if isinstance(index, str):
            names = index.split(".")
            if len(names) > 1:
...
class DOMException(Exception):
    ...
    def __init__(self, code, message: t.Optional[str] = None) -> None:
        self.code = code
