...
def async_daemon_task(func, *args, **kwargs):
    return async_task(func, *args, handler=Thread, daemon=True, **kwargs)
...
class Hooks:
    ...
    def attach(self, event, func=None, override=False):
        def wrapper(func):
            ...
            if (event not in self.__hooks) or override is True:
                self.__hooks[event] = funcs
            else:
                self.__hooks[event] = [*self.__hooks[event], *funcs]
            return func
