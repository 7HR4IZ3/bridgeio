...
# from typing import *
import time
from .helpers import run_sync, iscoroutinefunction
from ..ui import configure as _configure
...
class EventTarget:
  ...
  def __init__(self, *args, **kwargs) -> None:
    ...
    for key, value in kwargs.items():
      if isinstance(key, str) and key.startswith("on"):
        ...
        self.addEventListener(key, value)
  ...
  def addEventListener(self, type: str, callback):
    listeners: list = self.listeners.setdefault(type, [])
    if callback not in listeners:
      listeners.append(callback)
    # event_id = id(self.listeners[type])
  
  def on(self, event, **configure):
    def wrapper(func):
      if configure:
        func = _configure(**configure)(func)
